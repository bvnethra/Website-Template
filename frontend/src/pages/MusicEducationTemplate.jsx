...
import { musicData } from '../data/education/musicData';
...
function NavBar() {
  ...
  // Active state checker for relative paths
  const isActive = (path) => {
    const cleanPath = path === '/' ? '' : path.replace('/', '');
    const currentSubpath = location.pathname.split('/').filter(Boolean).slice(3).join('/');
...
// ---------------- MAIN ROUTER ----------------
export default function MusicEducationTemplate() {
  return (
