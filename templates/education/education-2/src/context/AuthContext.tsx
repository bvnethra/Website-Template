...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    const saved = localStorage.getItem('eduvora_auth_state');
    return saved === 'true';
  });

  const [role, setRole] = useState<UserRole>(() => {
    return (localStorage.getItem('eduvora_role') as UserRole) || 'student';
  });

  const [currentUser, setCurrentUser] = useState<StudentProfileData | null>(() => {
    const savedUser = localStorage.getItem('eduvora_current_user');
    if (savedUser) {
      try {
  ...
  const [token, setToken] = useState<string | null>(() => {
    return localStorage.getItem('eduvora_auth_token') || (isAuthenticated ? 'mock_jwt_token_eduvora_2026_xyz' : null);
  });

  const [loginHistory, setLoginHistory] = useState<LoginAuditSession[]>(() => {
    const savedHistory = localStorage.getItem('eduvora_login_sessions');
    if (savedHistory) {
      try {
  ...
  const [twoFactorEnabled, setTwoFactorEnabled] = useState<boolean>(() => {
    return localStorage.getItem('eduvora_2fa') === 'true';
  });

  useEffect(() => {
    localStorage.setItem('eduvora_auth_state', isAuthenticated ? 'true' : 'false');
    localStorage.setItem('eduvora_role', role);
    if (currentUser) {
    ...
    localStorage.setItem('eduvora_login_sessions', JSON.stringify(loginHistory));
    localStorage.setItem('eduvora_2fa', twoFactorEnabled ? 'true' : 'false');
  ...
  const login = async (id: string, pass: string, selectedRole: UserRole = 'student'): Promise<{ success: boolean; error?: string }> => {
    ...
    if (selectedRole === 'student') {
      ...
      const isMockStudent = (cleanId === 'EDV2026CS104' || cleanId.startsWith('EDV')) && (cleanPass === 'eduvora@2026' || cleanPass.length >= 6);
      
      if (!isMockStudent) {
        return { 
          success: false, 
          error: 'Invalid Student ID or Password. (Demo: EDV2026CS104 / eduvora@2026)' 
        };
      }
      ...
      const generatedToken = `eduvora_jwt_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
      ...
      const isMockFaculty = (cleanId === 'FAC2026CS01' || cleanId.startsWith('FAC')) && (cleanPass === 'eduvora@faculty' || cleanPass.length >= 6);
      
      if (!isMockFaculty) {
        return { 
          success: false, 
          error: 'Invalid Faculty Credentials. (Demo: FAC2026CS01 / eduvora@faculty)' 
        };
      }
      ...
      const generatedToken = `eduvora_fac_jwt_${Date.now()}`;
      setIsAuthenticated(true);
      setRole('faculty');
  ...
  const logout = () => {
    ...
    setToken(null);
    localStorage.removeItem('eduvora_auth_state');
    localStorage.removeItem('eduvora_auth_token');
  };
